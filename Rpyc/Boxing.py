...
from NetProxy import NetProxy, SyncNetProxy
...
def dump_exception(typ, val, tb):
    ...
    try:
        pickled_exc = pickle.dumps((typ, val, tbtext), PICKLE_PROTOCOL)
    except pickle.PicklingError, ex:
        newval = NestedException("pickling error %s\nexception type: %r\nexception object: %s" % (ex, typ, val))
        pickled_exc = pickle.dumps((NestedException, newval, tbtext), PICKLE_PROTOCOL)
    return pickled_exc
...
class Box(object):
    ...
    def _box(self, obj):
        ...
        elif isinstance(obj, NetProxy) and obj.__dict__["____conn"] is self.conn:
            return TYPE_LOCAL_PROXY, obj.__dict__["____oid"]
        elif isinstance(obj, tuple):
            if obj:
                return TYPE_TUPLE, [self._box(subobj) for subobj in obj]
            else:
                return TYPE_SIMPLE, ()
        elif isinstance(obj, ImmDict):
            if not obj.dict:
                return TYPE_SIMPLE, {}
            else:
    ...
    def pack(self, obj):
        ...
        return pickle.dumps(self._box(obj), PICKLE_PROTOCOL)
